...
RESULT_DUMP_PATH = (
    ...
    "arith_scalar_test/MaxUI/maxui_scalar_results.txt"
)
...
def run_single_test_scalar(jit_func, a, b, name, dtype):
    ...
    # ---------------- semantic check ----------------
    assert out == ref, (
        f"[{dtype}] maxui mismatch in case {name}\n"
        f"CPU={ref}, TPU={out}"
    )
...
@pytest.mark.parametrize(
    "a, b, name",
    make_maxui_u32_cases(),
)
def test_maxui_scalar_u32(a, b, name):
...
#rm -f sim* && source set-env && cmake --build ../iree-build/  && python -m pytest -vs /root/iree/tests/e2e/dlc_specific/test_set/arith_scalar_test/MaxUI/MaxUI.py::test_maxui_scalar_u32