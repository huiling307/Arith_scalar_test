...
RESULT_DUMP_PATH = (
    ...
    "arith_scalar_test/MinNumF/minnumf_scalar_results.txt"
)
...
def run_single_test_scalar(jit_func, a, b, name, dtype):
    ...
    assert ref_val == out_val, (
        f"[{dtype}] minnumf mismatch in case {name}\n"
...
@pytest.mark.parametrize(
    "a, b, name",
    make_minnumf_f32_cases(),
)
def test_minnumf_scalar_f32(a, b, name):
...
#rm -f sim* && source set-env && cmake --build ../iree-build/  && python -m pytest -vs /root/iree/tests/e2e/dlc_specific/test_set/arith_scalar_test/MinNumF/MinNumF.py::test_minnumf_scalar_f32